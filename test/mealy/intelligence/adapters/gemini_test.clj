...
(deftest parse-response-test
  ...
  (testing "Error response parsing (rate limit)"
    (let [json-body "{\"error\":{\"code\":429,\"message\":\"Resource has been exhausted (e.g. check quota).\"}}"
          resp {:status 429 :body json-body}
          parsed (gemini/parse-response resp)]
  ...
  (testing "Error response parsing (generic 400)"
    (let [json-body "{\"error\":{\"code\":400,\"message\":\"Bad Request\"}}"
