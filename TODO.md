...
- [x] **State Expansion:** Update `mealy.cell.core/make-cell` to initialize the expanded state map, including empty maps/sets for `:subscriptions`, `:handlers`, `:actions`, `:parent`, and `:children`. Update existing tests.
...
- [x] **Runtime Integration & Cleanup:** Wire the JVM Store and Bus together into a unified `mealy.runtime.jvm.core/start-node` entry point. Migrate all `sociocracy_integration_test.clj` assertions to use this new runtime. Delete the deprecated `mealy.shell.*` namespaces.
- [x] **EventBus Protocol Refinement:** Update `mealy.runtime.protocols/EventBus`. Refactor `register` to accept a topic (for discovery) rather than a cell ID. Add an `(unsubscribe [this topic id])` method. Update `query` to allow listing available topics. Update `mealy.runtime.jvm.bus` and `mealy.runtime.jvm.bus-test` to implement these changes.
- [x] **Network IO Abstraction:** Remove `hato` from the core dependencies and isolate it to the JVM runtime. Define a new, generic `[:http-request]` Action schema in `mealy.action.core`. Update the JVM worker pool (`mealy.runtime.jvm.core`) to intercept `[:http-request]` actions and execute them using `hato`, placing the resulting observation back on the Cell's input channel.
- [x] **Pure Adapter Refactor:** Refactor `mealy.intelligence.adapters.gemini` and `mealy.intelligence.adapters.llama`. Remove their stateful `go-loop` providers. Convert them into pure namespaces containing two functions: one to build the HTTP request map for the generic `[:http-request]` action, and one to parse the pure HTTP response observation into tokens and text.
- [x] **Pure Intelligence Routing:** Delete `mealy.intelligence.provider` and `mealy.intelligence.router` entirely. Move the provider scoring, budget tracking, and backoff logic into pure functions within a new `mealy.intelligence.core` namespace.
- [x] **Stateful Routing Integration:** Update the core Cell state (or the `default-handlers`) to manage provider budgets, rate limits, and backoff timestamps directly within the Cell's `:memory`. When a decision requires an LLM, the pure handler should use `mealy.intelligence.core` to select the best provider from memory, and yield an `[:http-request]` action instead of an `[:llm-request]`.
