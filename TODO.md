...
- [ ] **Action Registry Foundation:** Implement `mealy.action.core`. Define a multimethod `(defmulti execute (fn [action env] (:type action)))` to act as the extensible execution router. The `env` map will carry system contexts (e.g., `:gateway-chan`, `:cell-in-chan`).
